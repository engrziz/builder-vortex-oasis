...
const fallbackResponses: Record<string, string> = {
  ...
  'هلا': 'أهلاً وسهلاً! هل تر��د تعلم شيء جديد عن الأموال والأسهم؟ 🎈',
...
export default function FinanceChatbot() {
  ...
  const getAIResponse = async (userMessage: string): Promise<string> => {
    try {
      ...
      if (!response.ok) {
        console.warn(`AI API failed with status ${response.status}, using fallback responses`);
      ...
      if (data.error) {
        console.warn('API Error:', data.error, 'Using fallback responses');
        return getFallbackResponse(userMessage);
      }

      return data.response || getFallbackResponse(userMessage);
    } catch (error) {
      console.warn('Error calling AI API:', error, 'Using fallback responses');
      return getFallbackResponse(userMessage);
    }
  };
